...
//
// One @Observable singleton drives the chrome bell button (popover
...
private let log = Logger(subsystem: "dev.limpid", category: "notifications.history")
...
final class NotificationHistoryStore {
    ...
    /// Number of unread entries — surfaces as the chrome bell badge.
    var unreadCount: Int {
    ...
    private nonisolated static func write(_ entries: [NotificationEntry], to url: URL) {
        do {
            ...
            // Pretty output is Debug-only — Release builds skip the
            // formatting cost on every debounced write.
            #if DEBUG
