...
// Limpid — browser-style back/forward navigation history for the
...
//     forward stack to match browser semantics.
//   * The terminal column toolbar `< / >` buttons — call `navigateBack` /
...
extension WindowSession {
    ...
    /// Cap on how many "from where we came" entries we keep. Older
