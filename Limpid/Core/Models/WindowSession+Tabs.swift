...
// because it touches the container column selection invariant; everything that lives
...
extension WindowSession {
    ...
    ///     or it has neither (preserves the home-on-launch behavior).
    ...
    /// Open a tab in the currently-selected container (container column). Honors
    /// the user's actual container column selection rather than re-using the active
    ...
    /// container, or stays nil if the container is now empty so tab column
    ...
    /// Tabs in the given container — single entry point used by tab column to
    /// derive its row list. Routes to the specialized filter above.
    ...
    // MARK: - Cross-container move (tab column → container column drag-drop)

    /// Move the tab into a different container. Used by the tab column → container column
    ...
    ///      the destination tab column list)
    ...
    func moveTab(_ tabID: UUID, to target: ContainerID) {
        ...
        // — otherwise a later `activateProject` / `activateGroup`
        // sees the stale id and silently jumps to whichever container
        // the tab landed in. `closeTab` already does the equivalent
        // sweep; symmetry here keeps the
        // `lastActiveTabID ∈ tabs(in: thisContainer)` invariant.
        forgetLastActive(tabID: tabID, container: sourceContainer)
        ...
        // vacated slot (activate the tab to the right); empty
