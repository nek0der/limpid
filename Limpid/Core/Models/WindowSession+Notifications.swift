...
// PaneState directly, so the bell indicators on TabRow, container column
// ContainerRow, the toolbar bell button, and the dock badge stay
// consistent.
...
extension WindowSession {
    ...
    /// Used by Project headers in container column, including when collapsed.
    ...
    /// numeric badge on the toolbar bell button. Reads the cache
    /// directly so observers (DockBadgeSync, toolbar) only re-evaluate
