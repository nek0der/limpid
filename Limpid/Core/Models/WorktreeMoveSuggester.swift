...
private let log = Logger.limpid("worktree.move.suggester")
...
final class WorktreeMoveSuggester {
    ...
    func handleEvent(_ record: CwdEventRecord) {
        ...
        Task { [weak self] in
            ...
            // `Task { }` inside this `@MainActor` method inherits the
            // MainActor isolation, so the suspension on
            // `resolveUnregisteredWorktree` (a `nonisolated static`)
            // resumes back on the main actor automatically. The
            // explicit `MainActor.run` here was a redundant extra
            // hop that read as "we're crossing actors" when in fact
            // we are not.
