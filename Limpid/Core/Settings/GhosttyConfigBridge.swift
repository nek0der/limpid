...
enum GhosttyConfigBridge {
    ...
    /// changes when embedded (cmux#2922 / ghostty#11017), so we pick
    /// one side at config-write time and re-write on appearance
    /// change. The live-switch path runs through `reloadConfig`.
    ...
    static func makeConfigString(
        ...
        // cell foreground + ANSI palette track macOS Terminal.app (same
        // choice as cmux). Must come AFTER `resources-dir` and uses an
