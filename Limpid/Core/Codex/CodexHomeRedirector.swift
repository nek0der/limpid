...
//
// The shadow-home indirection lets us inject Limpid's lifecycle
...
private let log = Logger.limpid("codex.home.redirector")
...
final class CodexHomeRedirector {
    ...
    ///    OSC 2 title — Limpid mirrors that title into the tab column sidebar
    ...
    /// because we control every byte: the exact serialization must
    ...
    /// Canonicalized `/bin/sh '<path>'` invocation for the worktree
    ...
    /// the byte-identical command for Codex to recognize the hook as
