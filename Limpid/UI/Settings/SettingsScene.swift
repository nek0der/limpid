...
// so the chrome feels integrated, not stuck above the sidebar.
//
// Settings hosts itself in `Window(id:)` + `.windowStyle(
// .hiddenTitleBar)` (see LimpidApp). `limpidSettingsChrome()`
...
struct SettingsScene: View {
    ...
    /// Slab width — matches the proportions of the main window's L1.
    ...
    var body: some View {
        ZStack(alignment: .topLeading) {
            ...
            // Same `liquidGlassSlab` + insets the main window uses
            // on L1, so the visual rhythm matches across windows.
    ...
    static var leadingInset: CGFloat {
        LimpidLayout.l1InsetH + sidebarWidth + 8
    }
...
/// traffic-light row, then the section list. Mirrors `L1SlabContent`
/// but slimmed down — Settings doesn't need an in-slab chrome row.
