...
// Limpid — horizontal tab strip shown above terminal column when the user toggles
// the tab layout. Reuses the exact same `TabRow` the vertical tab column list
...
struct HorizontalTabBar: View {
    ...
    var body: some View {
        ...
        GeometryReader { geo in
            // Width the strip needs to honor the per-tab minimum.
    ...
    private func strip(_ tabs: [Tab], pillWidth: CGFloat?) -> some View {
        ...
        // strip render, mirroring `TabsListView` — computing it inside
        // each `paneMergeDropTarget` would re-run `tab(containing:)` per
        // tab × per drag-frame. Detach-into-new-tab is intentionally NOT
        // wired here: the horizontal strip has no unambiguous empty
        // region to catch it, and the right-click menu / vertical list
        // already cover that path.
        let paneDragSourceTabID = paneDragSourceTabID()
        return HStack(spacing: LimpidLayout.horizontalTabSpacing) {
            ForEach(tabs) { tab in
                TabRow(
                    tab: tab,
                ...
                // Same pane-merge drop as the vertical tab column list: dropping
                // a ⌥⌘-dragged pane on another tab's pill merges it in.
                // `pillHorizontalPadding: 0` matches the strip's own
                // `TabRow(pillHorizontalPadding: 0)` so the drop
                // highlight overlays the active pill exactly instead
                // of insetting itself 10pt like the vertical list.
                .paneMergeDropTarget(
                    targetTabID: tab.id,
                    session: session,
                    dragState: dragState,
        ...
        // `matchedGeometryEffect` in `paneMergeDropTarget` needs a
        // value-keyed animation to glide the accent rect between pills as
        // the cursor crosses neighbours instead of snapping per pill.
    ...
    /// Tab id owning the pane currently being dragged, or nil when no
    /// pane drag is in flight. Mirrors `TabsListView.paneDragSourceTabID`
    /// so the source pill refuses the drop instead of showing the green
    /// `+` Copy badge.
    private func paneDragSourceTabID() -> UUID? {
