...
// `NSWindow` to SwiftUI so we can apply Liquid Glass chrome tweaks.

import AppKit
import OSLog
import SwiftUI

private let log = Logger(subsystem: "dev.limpid", category: "window.chrome")
...
struct WindowAccessor: NSViewRepresentable {
    ...
    func updateNSView(_ nsView: NSView, context: Context) {
        (nsView as? AccessorView)?.configure = configure
    }
...
/// Slide the three traffic-light buttons inward to land inside the L1
...
/// card inset by `LimpidLayout.l1InsetH`. The offset puts the
...
extension View {
    /// Floating Liquid Glass chrome for the main terminal window. The
    /// L1 sidebar is a floating card inset by `LimpidLayout.l1InsetH`,
    ...
    func limpidWindowChrome(onWindow: ((NSWindow) -> Void)? = nil) -> some View {
        background(WindowAccessor(repositionsTrafficLights: true) { window in
            onWindow?(window)
            applyTransparentTitleChrome(to: window, clearBackground: true)
            // L1 slab inset is 10pt + 10pt corner radius → push the
            // close button to (26, 22) so it lands inside the slab.
            repositionTrafficLights(in: window)
        })
    }

    /// Flush-sidebar chrome for the Settings window. Same transparent
    ...
    /// Settings pattern).
    func limpidSettingsChrome(onWindow: ((NSWindow) -> Void)? = nil) -> some View {
        background(WindowAccessor(repositionsTrafficLights: true) { window in
            onWindow?(window)
...
/// Shared chrome setup: transparent title bar so the SwiftUI content
...
@MainActor
private func applyTransparentTitleChrome(to window: NSWindow, clearBackground: Bool = true) {
    if clearBackground {
