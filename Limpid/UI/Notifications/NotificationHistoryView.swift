...
// fired. Reachable from either bell button (sidebar / toolbar capsule)
// and from `⌘⇧N`.
...
private struct NotificationHistoryRow: View {
    ...
    private var kindIcon: some View {
        ...
        return Image(systemName: name)
            ...
            // `.accessibilityLabel(_:)` resolves a literal expression
            // through `LocalizedStringKey`, but a ternary collapses
            // the result to plain `String` and silently bypasses the
            // catalog. Branch on the modifier so both labels route
            // through xcstrings.
