// TempGitRepo.swift
// Disposable git repository under `tmp`, fully initialized with one
// empty commit so `git` subcommands have a valid HEAD to compare
// against. Use this from smoke tests instead of the real Limpid repo
// when the test needs to mutate state (commits, worktrees, …).
