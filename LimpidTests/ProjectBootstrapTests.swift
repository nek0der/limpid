...
// Limpid — Covers the on-disk shape of `Project.bootstrap` and
// `BootstrapItem`. The Codable contract is the wedge: a `state.json`
// written before `bootstrap` existed must still load (forward-compat),
// and the `oneOf` shorthand/detailed encoding must round-trip both
// sides.
...
struct ProjectBootstrapTests {
    ...
    func projectDecodes_legacyStateJsonWithoutBootstrap() throws {
        // Snapshot of how a Project was serialised before this PR
        // added the bootstrap field. The loader must not fail here —
        // a `keyNotFound("bootstrap")` would lose every existing user
