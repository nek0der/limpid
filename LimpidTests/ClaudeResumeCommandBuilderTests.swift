...
struct ClaudeResumeCommandBuilderTests {
    ...
    @Test("resumeCommand drops the resume (no shell injection) for a sessionId with metacharacters")
    func resumeCommand_rejectsShellInjectionInSessionId() {
        // A tampered state.json could carry a crafted sessionId; it must never
        // be interpolated into the shell command.
        let cmd = ClaudeResumeCommandBuilder.resumeCommand(sessionId: "x; rm -rf ~ #")
        #expect(cmd == "claude")
        #expect(!cmd.contains("rm"))
    }

    @Test("resumeCommand keeps the cd but drops the resume for an invalid sessionId")
    func resumeCommand_invalidSessionId_keepsCwdDropsResume() {
        let cmd = ClaudeResumeCommandBuilder.resumeCommand(sessionId: "bad id", cwd: "/tmp/repo")
        #expect(cmd == "cd '/tmp/repo' && claude")
    }
}
