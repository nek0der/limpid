...
// Limpid — tests for the Codex resume command builder.
...
struct CodexResumeCommandBuilderTests {
    ...
    func rejectsShellInjectionInSessionId() {
        let cmd = CodexResumeCommandBuilder.resumeCommand(sessionId: "x; rm -rf ~ #")
        #expect(cmd == "codex")
        #expect(!cmd.contains("rm"))
    }

    @Test("resumeCommand keeps the cd but drops the resume for an invalid sessionId")
    func invalidSessionId_keepsCwdDropsResume() {
        let cmd = CodexResumeCommandBuilder.resumeCommand(sessionId: "bad id", cwd: "/tmp/repo")
        #expect(cmd == "cd '/tmp/repo' && codex")
    }
}
