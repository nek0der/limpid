...
private func parse(_ data: Data) -> Result<AgentMirrorRequest, AgentMirrorRequest.Rejection> {
    Result { () throws(AgentMirrorRequest.Rejection) in
        try AgentMirrorRequest.parse(data, ownSocketName: ownSocketName)
    }
}
...
struct AgentMirrorRequestTests {
    ...
    @Test func paneEnvironment_namesTheDirectoryOnlyWhenHosting() {
        let host = PaneShellEnvironment.agentTmuxHost(
            ...
            mirrorRequestsDirectory: URL(fileURLWithPath: "/private/tmp/requests", isDirectory: true)
        )
...
private final class WatcherHarness {
    ...
    init(root: URL) {
        ...
        watcher = AgentMirrorRequestWatcher(
            directory: directory,
            ownSocketName: ownSocketName,
            hasLeaf: { [record] in record.leaves.contains($0) },
            open: { [record] in record.opened.append($0) }
        )
    }
...
struct AgentMirrorRequestWatcherTests {
    ...
    /// relaunch, is served when watching starts.
    @Test func start_servesRequestsAlreadyThere_andRemovesThem() throws {
        try withTempDir { root in
            let harness = WatcherHarness(root: root)
            defer { harness.watcher.stop() }
    ...
    /// Another user could drop requests into a directory they can write.
    @Test func sharedDirectory_isNotWatched() throws {
        try withTempDir { root in
            let harness = WatcherHarness(root: root)
            defer { harness.watcher.stop() }
            try harness.write(requestJSON(), name: "e.json")
            try FileManager.default.setAttributes([.posixPermissions: 0o777], ofItemAtPath: harness.directory.path)

            harness.watcher.start()

            #expect(harness.opened.isEmpty)
            #expect(harness.contents() == ["e.json"])
        }
    }
}
